...
- `src/scripts` contains the `Leveling` runtime and statistics, extracted `AreaRepository`, `Navigator`, `RoomScanner`, and `Combat` components, and `BuffManager`, plus `scripts.json`.
...
1. `Stats`, to separate the remaining statistics storage and reporting from session orchestration.
2. Trigger lifecycle cleanup and evaluation of whether a small `TriggerManager` is useful.
3. `Commands`, if alias handling grows beyond the current thin adapters.
